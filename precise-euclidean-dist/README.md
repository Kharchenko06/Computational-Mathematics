...
# CLI-демо (3-4-5 и катастрофическое сокращение значащих бит)
make demo
...
`Naive` дает ~10 ULP на катастрофической отмене. `KBN3`/`KBN4` снижают до 0–1 ULP.
...
- Катастрофическая сокращение значающих бит: `A = (1e15,…)`, `B = (1e15+1,…)`, ожидается `sum = 3.0`
...
### `test_euclidean_full`
- 16 пар точек: `R^2`, `R^3`, `R^5`, катастрофическая сокращение значающих бит, экстремальные значения
...
**Когда нужен `sqrt_refined`?** При катастрофической отмене `std::sqrt(cs.sum)` дает до ~5–10 ULP; шаг Ньютона с FMA-точным остатком возвращает ошибку в пределы 1 ULP.
